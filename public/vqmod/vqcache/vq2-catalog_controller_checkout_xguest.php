<?php
class ControllerCheckoutXGuest extends Controller {
	public function index() {
		...
		} elseif (isset($this->session->data['guest']['payment']['country_id'])) {
		...
		} elseif (isset($this->session->data['guest']['payment']['zone_id'])) {
		...
		foreach ($this->model_account_xcustomer->getCustomOptions() as $option) {
		...
		$this->data['text_select'] = $this->language->get('text_select');
		$this->data['optionsP'] = $this->model_account_xcustomer->getCustomOptions(1);
		$this->data['optionsA'] = $this->model_account_xcustomer->getCustomOptions(2);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/xguest.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/checkout/xguest.tpl';
		} else {
	...
	public function validate() {
		...
		if (!$json) {
			...
			foreach ($this->model_account_xcustomer->getCustomOptions() as $option) {
				...
				if ( $isActive   && $modData['mob_req_checkout'] && $modData['mob_show_checkout'] && $modData['mob_fix'] && ((utf8_strlen($this->request->post['telephone']) != $modData['mob_fix']))){
					$json['error']['telephone'] = $json['error']['telephone'] = $this->language->get('error_telephone');
				}else if ( $isActive  && $modData['mob_req_checkout'] && $modData['mob_show_checkout'] && !$modData['mob_fix'] && $modData['mob_min'] && $modData['mob_max'] && ((utf8_strlen($this->request->post['telephone']) < $modData['mob_min']) || (utf8_strlen($this->request->post['telephone']) > $modData['mob_max']))){
					$json['error']['telephone'] = $json['error']['telephone'] = $this->language->get('error_telephone');
				}else if ($isActive && $modData['mob_req_checkout'] && $modData['mob_show_checkout'] && !$modData['mob_max'] && !$modData['mob_fix'] && !$modData['mob_min'] && ((utf8_strlen($this->request->post['telephone']) < 3) || (utf8_strlen($this->request->post['telephone']) > 32))) {
		...
		if (!$json) {
			foreach ($this->model_account_xcustomer->getCustomOptions() as $option) {
				...
				if(isset($this->request->post['option'.$option['option_id']]))
					...
					$this->session->data['guest']['payment']['option'.$option['option_id']] = rtrim($valueX,',');
				}
			}
			$this->load->model('account/xcustomer');
			foreach ($this->model_account_xcustomer->getCustomOptions(0) as $option) {
				...
				if($option['section']==1)
				$this->session->data['xtensions']['personal'][$option['identifier']]=$this->model_account_xcustomer->getGuestOptionsA($option['option_id'],$option['type'],$this->session->data['guest']['payment']['option'.$option['option_id']]);
				else
				$this->session->data['xtensions']['payment'][$option['identifier']]=$this->model_account_xcustomer->getGuestOptionsA($option['option_id'],$option['type'],$this->session->data['guest']['payment']['option'.$option['option_id']]);
				else
				if($option['section']==1)
				$this->session->data['xtensions']['personal'][$option['identifier']]=$this->session->data['guest']['payment']['option'.$option['option_id']];
				else
				$this->session->data['xtensions']['payment'][$option['identifier']]=$this->session->data['guest']['payment']['option'.$option['option_id']];
			}
